from REACTRL.img_modules.img_data_2_target import train_loader, test_loader
...
def func(X, model=None, method='resnet18'):
    lr = 10 ** X[0, 0]
    output_dir='output_target_2_%s_%s_%s' % (lr, method, datetime.now().strftime("%Y%m%d-%H%M"))
    if model is None:
        model = ResNet18(1, 2)
