from REACTRL.img_modules.img_data_3_class import train_loader, test_loader
...
def func(X, model=None, method='resnet18'):
    lr = 10 ** X[0, 0]
    output_dir='output_product_3_%s_%s_%s' % (lr, method, datetime.now().strftime("%Y%m%d-%H%M"))
    if model is None:
        model = ResNet18(1, 3)
